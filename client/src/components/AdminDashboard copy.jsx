...
export default function AdminDashboard() {
  ...
  const openModal = (type, data = null) => {
    ...
    if (activeSidebarPage === 'studs' && (type === 'add' || type === 'edit')) {
      setFormData(data ? { ...normalized, images: [], branches: data.branches || data.Branches || [] } : { country: 'مصر', images: [], branches: [] });
      setCurrentStep(1);
      setFormErrors({});
    } else if (activeSidebarPage === 'auctions' && (type === 'add' || type === 'edit')) {
      setFormData(data ? { ...normalized, venues: data.venues || data.Venues || [{ studName: '', branch: '', horseIds: [] }] } : { venues: [{ studName: '', branch: '', horseIds: [] }] });
      setCurrentStep(1);
      setFormErrors({});
    } else {
  ...
  const handleNextStep = (e) => {
    ...
    if (activeSidebarPage === 'auctions') {
      if (currentStep === 1) {
        ...
        if (!formData.venues || formData.venues.length === 0 || !formData.venues[0].horseIds || formData.venues[0].horseIds.length === 0) {
          errors.venues = true;
        }
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (activeSidebarPage === 'studs') {
        ...
        studForm.append('Description', formData.about || '');
        studForm.append('Email', formData.email || '');
        studForm.append('Phone', formData.phone || '');
        studForm.append('City', formData.city || '');
        studForm.append('StudType', formData.type || '');
        if (formData.foundedDate) studForm.append('EstablishedDate', formData.foundedDate);

        // Handle image - check for either name 'image' or 'images' depending on modal
        if (formData.image) studForm.append('ImageFile', formData.image);
        else if (formData.images && formData.images.length > 0) studForm.append('ImageFile', formData.images[0]);

        if (modalConfig.type === 'add') {
          await studApi.create(studForm);
        } else {
          await studApi.update(formData.id || formData.Id, studForm);
        }
      } else if (activeSidebarPage === 'auctions') {
        // ... (Auction logic already updated in previous turn, keeping it same)
        const auctionForm = new FormData();
        auctionForm.append('Name', formData.title || '');
        auctionForm.append('StartTime', formData.startDate);
        auctionForm.append('EndTime', formData.endDate);
        auctionForm.append('BasePrice', formData.basePrice || 0);
        auctionForm.append('MinimumIncrement', formData.minIncrement || 1000);
        auctionForm.append('Description', formData.description || '');

        const venue = (formData.venues && formData.venues[0]) || {};
        const microchipId = (venue.horseIds && venue.horseIds[0]);
        if (microchipId) auctionForm.append('MicrochipId', microchipId);

        if (formData.image) auctionForm.append('ImageFile', formData.image);

        if (modalConfig.type === 'add') {
          await auctionApi.createAuction(auctionForm);
        } else {
          await adminApi.updateAuction(formData.id || formData.Id, {
            Name: formData.title,
            StartTime: formData.startDate,
            EndTime: formData.endDate,
            BasePrice: formData.basePrice,
            MinimumIncrement: formData.minIncrement,
        ...
        if (modalConfig.type === 'add') {
          ...
          horseForm.append('MicrochipId', formData.microchipId || formData.MicrochipId);
          horseForm.append('Name', formData.name);
          horseForm.append('Age', formData.age);
          horseForm.append('Gender', formData.gender || formData.type || 'Male');
          horseForm.append('Breed', formData.breed);
          horseForm.append('StudName', formData.studName);
          horseForm.append('ColorName', formData.color);
          horseForm.append('Sire', formData.sire);
          horseForm.append('Dam', formData.dam);
          horseForm.append('Breeder', formData.breeder);

          if (formData.image) horseForm.append('ImageFile', formData.image);
          if (formData.healthRecordPdf) horseForm.append('HealthReportFile', formData.healthRecordPdf);
          if (formData.pedigreeImg) horseForm.append('PedigreeImageFile', formData.pedigreeImg);
          ...
          const horsePayload = {
            ...
            Gender: formData.gender || formData.type,
            Breed: formData.breed,
            ImageUrl: typeof formData.image === 'string' ? formData.image : null,
            Breeder: formData.breeder,
            MicrochipId: formData.id || formData.microchipId || formData.MicrochipId
          };
          await adminApi.updateHorse(horsePayload.MicrochipId, horsePayload);
        }
      } else if (activeSidebarPage === 'dashboard' || !activeSidebarPage) {
        if (modalConfig.type === 'add') {
          const userForm = new FormData();
          userForm.append('FullName', formData.name || formData.fullName || formData.seller);
          userForm.append('Email', formData.email);
          userForm.append('Password', 'Password123!');
          const role = activeTab === 'البائعين' ? 'Seller' : activeTab === 'المشترين' ? 'Buyer' : 'EquineVet';
          ...
          if (role === 'Seller') {
            ...
            userForm.append('ExperienceYears', formData.experienceYears || 0);
            userForm.append('Address', formData.address || '');
            if (formData.nationalIdFile) userForm.append('NationalIdFile', formData.nationalIdFile);
            if (formData.recommendationLetter) userForm.append('RecommendationLetter', formData.recommendationLetter);
          } else if (role === 'EquineVet') {
            userForm.append('CountryCity', formData.countryCity || '');
            userForm.append('LicenseNumber', formData.license || '');
            userForm.append('VetSpecialization', formData.specialty || '');
            userForm.append('ExperienceYears', formData.experienceYears || 0);
            userForm.append('ClinicsWorkedAt', formData.clinicsWorkedAt || '');
            userForm.append('VetBio', formData.vetBio || '');
            if (formData.licenseFile) userForm.append('LicenseFile', formData.licenseFile);
            if (formData.nationalIdFile) userForm.append('NationalIdFile', formData.nationalIdFile);
            if (formData.vetCertificates) userForm.append('VetCertificates', formData.vetCertificates);
          }

          await userApi.register(userForm);
        } else {
          const userUpdatePayload = {
            FullName: (activeTab === 'البائعين' ? formData.seller : formData.name) || formData.fullName || formData.FullName,
          ...
          const userId = formData.id || formData.Id;
          console.log('Updating user:', userId, userUpdatePayload);
          const response = await adminApi.updateUser(userId, userUpdatePayload);
      ...
      {modalConfig.isOpen && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`bg-white rounded-2xl shadow-2xl w-full max-h-[95vh] overflow-y-auto animate-fade-in-up custom-scrollbar ${((activeSidebarPage === 'studs') || (activeSidebarPage === 'horses' && modalConfig.type === 'edit') || activeSidebarPage === 'requests' || activeSidebarPage === 'dashboard') ? 'max-w-5xl' : 'max-w-3xl'}`}>
            ...
            {(modalConfig.type === 'add' || modalConfig.type === 'edit') && activeSidebarPage === 'studs' && (
              <div className="flex flex-col w-full">
                <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gradient-to-l from-emerald-900 to-emerald-800 shrink-0 rounded-t-2xl">
                  <h2 className="text-2xl font-black text-white flex items-center gap-3">
                    {modalConfig.type === 'edit'
                      ? <><i className="fas fa-edit text-emerald-200"></i> ����� ������ ������</>
                      : <><i className="fas fa-plus-circle text-emerald-200"></i> ����� ���� ����</>
                    }
                  </h2>
                ...
                {/* ���� ������� */}
                <div className="flex items-center justify-center gap-2 px-8 pt-6 pb-2 bg-[#FAF9F6]">
                  {[
                    { num: 1, label: '�������� ��������' },
                    { num: 2, label: '�������' },
                    { num: 3, label: '������� ���������' },
                    ...
                    <React.Fragment key={step.num}>
                      <div className="flex flex-col items-center gap-2 px-2">
                        <div className={`w-8 h-8 flex items-center justify-center text-sm font-bold transition-all duration-300 rounded-lg border
              ${currentStep >= step.num
                            ? 'bg-emerald-600 border-emerald-600 text-white shadow-md'
                            : 'bg-white text-gray-400 border-gray-200'}`}>
                          {step.num}
                        </div>
                        <span className={`text-xs transition-colors ${currentStep >= step.num ? 'text-emerald-800 font-bold' : 'text-gray-400'}`}>
                          {step.label}
                        </span>
                      </div>
                      {idx < arr.length - 1 && (
                        <div className="flex-1 h-[2px] bg-gray-200 mb-4 relative">
                          <div
                ...
                {currentStep === 4 && (
                  <div className="space-y-6 max-w-5xl mx-auto py-2">
                    <div className="space-y-2 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:border-emerald-500 transition-colors">
                      <label className="text-sm font-bold text-emerald-800 block">رفع صورة المربط {modalConfig.type === 'add' && <span className="text-red-500">*</span>}</label>
                      <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors ${formErrors.image ? 'border-red-500 bg-red-50' : ''}`}>
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                          ...
                          <p className="text-xs text-gray-400">الصيغة المدعومة : صورة فقط (image/*)</p>
                        </div>
                        <input type="file" name="image" className="hidden" accept="image/*" onChange={handleInputChange} />
                      </label>
                      {formData.image && (
                        <div className="mt-4 w-32 h-32 rounded-xl border-2 border-emerald-600 overflow-hidden shadow-md">
                          <img src={formData.imagePreview || formData.image} alt="Stud Preview" className="w-full h-full object-cover" />
                        </div>
                      )}
                    </div>
          ...
          <div className="p-6 border-t border-gray-100 bg-white flex justify-end gap-4 rounded-b-[2rem]">
            ...
            {currentStep < 4 ? (
              <button onClick={handleNextStep} className="px-10 py-3.5 rounded-2xl font-bold text-white bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2">التالي</button>
            ) : (
              <button onClick={handleSubmit} className="px-10 py-3.5 rounded-2xl font-bold text-white bg-emerald-800 hover:bg-emerald-900 shadow-lg shadow-emerald-900/20 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2">
                {modalConfig.type === 'edit' ? <><i className="fas fa-save"></i>{' '}حفظ التعديلات</> : 'حفظ البيانات'}
              </button>
            )}
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="col-span-2">
                ...
                {formData.image && (
                  <div className="mt-3 w-24 h-24 rounded-xl overflow-hidden border-2 border-emerald-600 shadow-md">
                    <img src={formData.imagePreview || formData.image} alt="Preview" className="w-full h-full object-cover" />
                  </div>
                )}
              </div>
              ...
              <div className="col-span-2">
                ...
                {formData.pedigreeImg && (
                  <div className="mt-3 w-24 h-24 rounded-xl overflow-hidden border-2 border-emerald-600 shadow-md">
                    <img src={formData.pedigreeImgPreview || formData.pedigreeImg} alt="Pedigree Preview" className="w-full h-full object-cover" />
                  </div>
                )}
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="">
            {currentStep === 1 && (
              <div className="grid grid-cols-2 gap-5">
                ...
                <div><label className="block text-sm font-bold text-gray-700 mb-2">السعر الافتتاحي (Base Price)</label><input required type="number" name="basePrice" value={formData.basePrice || ''} onChange={handleInputChange} className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50" placeholder="مثال: 50000" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">أقل زيادة للمزايدة (Minimum Increment)</label><input required type="number" name="minIncrement" value={formData.minIncrement || '1000'} onChange={handleInputChange} className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50" /></div>
            ...
            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <input required={modalConfig.type === 'add'} name={activeTab === 'البائعين' ? 'seller' : 'name'} value={(activeTab === 'البائعين' ? formData.seller : formData.name) || ''} onChange={handleInputChange} className="w-full border border-gray-200 rounded-xl p-3 bg-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition" placeholder="أدخل الاسم..." />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">البريد الإلكتروني *</label>
                  <input required={modalConfig.type === 'add'} type="email" name="email" value={formData.email || ''} onChange={handleInputChange} className="w-full border border-gray-200 rounded-xl p-3 bg-white text-left focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition" dir="ltr" placeholder="example@mail.com" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">رقم الهاتف *</label>
                  <input required={modalConfig.type === 'add'} name="phone" value={formData.phone || ''} onChange={handleInputChange} className="w-full border border-gray-200 rounded-xl p-3 bg-white text-left focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition" dir="ltr" placeholder="+20 100 000 0000" />
                </div>
                <div>
...
<aside className="w-64 bg-gray-900 text-gray-300 flex flex-col justify-between hidden md:flex z-10 shrink-0">
  ...
  <div onClick={() => navigate('/profile')} className="p-4 bg-gray-800 flex items-center justify-between m-4 rounded-xl cursor-pointer hover:bg-gray-700 transition">
    <div>
      <h4 className="text-white text-sm font-semibold">{JSON.parse(localStorage.getItem('user'))?.fullName || JSON.parse(localStorage.getItem('user'))?.FullName || 'مدير النظام'}</h4>
            ...
            <div className="absolute top-8 left-0 w-80 bg-white shadow-2xl rounded-xl border border-gray-100 z-50 overflow-hidden transform transition-all">
              ...
              <div className="max-h-80 overflow-y-auto">
                {registrationRequests.length > 0 ? (
                  registrationRequests.slice(0, 5).map(req => (
                    <div key={req.id} className="p-4 border-b border-gray-50 hover:bg-emerald-50/50 cursor-pointer transition flex gap-4">
                      <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-500 flex items-center justify-center shrink-0"><i className="fas fa-user-plus"></i></div>
                      <div><p className="text-sm text-gray-800 font-medium">طلب تسجيل جديد من <span className="font-bold">{req.fullName}</span></p><p className="text-xs text-gray-400 mt-1">{req.date}</p></div>
                    </div>
                  ))
                ) : (
              ...
              <div onClick={() => { setActiveSidebarPage('requests'); setIsNotifOpen(false); }} className="p-3 bg-gray-50 text-center border-t border-gray-100 cursor-pointer hover:bg-gray-100"><span className="text-sm text-gray-600 font-medium">عرض كل الطلبات</span></div>
            </div>
          )}
        </div>
      ...
      <div className="animate-fade-in-up">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div><p className="text-gray-500 text-sm mb-2 font-medium">إجمالي المستخدمين</p><h3 className="text-3xl font-bold text-gray-800">{(sellersData.length + buyersData.length + vetsData.length).toLocaleString('ar-EG')}</h3><p className="text-green-500 text-xs mt-2 font-medium"><i className="fas fa-arrow-up ml-1"></i>حسابات نشطة</p></div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div><p className="text-gray-500 text-sm mb-2 font-medium">إجمالي الخيول</p><h3 className="text-3xl font-bold text-gray-800">{(horsesData.length).toLocaleString('ar-EG')}</h3><p className="text-green-500 text-xs mt-2 font-medium"><i className="fas fa-horse ml-1"></i>خيول مسجلة</p></div>
            <div className="bg-emerald-50 w-14 h-14 flex items-center justify-center rounded-2xl"><i className="fas fa-horse-head text-emerald-600 text-2xl"></i></div>
          </div>
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div><p className="text-gray-500 text-sm mb-2 font-medium">طلبات قيد الانتظار</p><h3 className="text-3xl font-bold text-gray-800">{(registrationRequests.length).toLocaleString('ar-EG')}</h3><p className="text-red-500 text-xs mt-2 font-medium"><i className="fas fa-clock ml-1"></i>طلبات جديدة</p></div>
